...
---
## Analyst Decision Prompt – Isolate Endpoint (Yes Path)
![Userprompt_yes](https://imgur.com/LAWZzNe.png)
When a high-confidence detection was generated by **LimaCharlie**, the workflow paused and presented an interactive decision prompt inside **Tines**. This prompt surfaced all the critical context an analyst would need before taking a containment action.

The prompt displayed:
- Detection title identifying the threat (**HackTool – LaZagne**)
- Timestamp of the activity
- Affected host name
- Source IP address
- Username involved
- File path and exact command line used
- Sensor ID for traceability
- A direct link back to the detection in **LimaCharlie** for deeper investigation

After reviewing this information, I selected **“Yes”** to proceed with isolating the machine. This simulates a real SOC decision point where an analyst confirms that the risk justifies containment before taking an impactful action.

Submitting the prompt triggered the automated response branch of the workflow, which:
- Sent an isolation request to **LimaCharlie** via API
- Began tracking the isolation status
- Reported the action and outcome through **Slack** for visibility

This approach balances automation with human judgment, ensuring isolation actions are intentional, auditable, and based on clear evidence rather than fully automatic enforcement.

---
## Endpoint Isolation Validation – Network Connectivity Loss
![Isolation_validation](https://imgur.com/zQAkhTC.png)

After approving the isolation action in the **Tines** user prompt, I validated the outcome directly on the affected Windows endpoint to confirm that the response worked as intended.

Before isolation, the system had an active, continuous ping running to an external IP address to establish a clear baseline for network connectivity. Once the isolation request was executed through **LimaCharlie**, I observed an immediate change in behavior on the endpoint.

As shown in the screenshot, the ping responses transitioned from successful replies to repeated **“General failure”** messages. This confirmed that the endpoint was no longer able to communicate externally, indicating that network isolation had been successfully enforced.
